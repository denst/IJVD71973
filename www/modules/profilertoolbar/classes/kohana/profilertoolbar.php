...
//    return $tmp['current']['memory'];
  ...
  public static function add_log($name, $data = '', $debug = false)
  {
      $directory = Request::current()->directory();
      $controller = Request::current()->controller();
      $action = Request::current()->action();
      $requst_path = '/'.$directory.'/'.$controller.'/'.$action;
      ini_set('log_errors', true);
      ini_set('error_log', MODPATH.'profilertoolbar/errors/errors.log');
      error_log("------------------------------------------------------");
      if($debug)
          $data = Debug::vars($data);
      error_log($name.' - '.$data.' - '.$requst_path);
      return true;
  }
  
  public static function clear_file()
  {
      $handle = fopen(MODPATH.'profilertoolbar/errors/errors.log', "w+'");
      return false;
  }
}