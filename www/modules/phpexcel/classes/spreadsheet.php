...
class Spreadsheet
{
	...
	public function send($settings=array())
	{
		...
		$request = Response::factory(); //Request::current();
        $request->headers(array('Content-Type' => $mime.';charset=utf8', 'Content-Disposition' => 'attachment;filename="'.$settings['name'].'.'.$ext.'"','Cache-Control' => 'max-age=0'));
